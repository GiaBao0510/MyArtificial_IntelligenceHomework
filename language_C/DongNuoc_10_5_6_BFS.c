...
//Ngan xep
	...
	//in nut o dau hang doi
	node Front(queue Q){
		return Q.data[Q.front];
	}
	//Kiem tra trang thai da ton tai trong hang doi
	int compareStateInQueue(queue Q,state X){
		while(!Empty(Q)){
			if(compareState(X, Front(Q)->TH) == 1){
				return 1;	//Da ton tai
			}else{
				pop(&Q);
			}
...
//Duyet Bai toan theo chieu sau
		...
		while(!Empty(open)){
			...
			for(opt = 1;opt <=6;opt ++){
				...
				if(callOperation(current,&result,opt) == 1){
					...
					if(compareStateInQueue(open,result) == 1 || compareStateInQueue(closed,result) == 1){
						continue; 
					}
	...
	void cachGiai(node P){
		...
		for(P;P != NULL; P = P->next){
			//printState(P->TH);
			copyOtherState(P->TH,&KQ[dem]);
			KQ[dem].opt = P->operation;
			dem++;
		}
		int i;
		for(i= dem -1 ; i>=0;i--){
