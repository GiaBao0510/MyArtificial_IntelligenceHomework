#include<stdio.h>
#include<stdlib.h>
#include<queue>					//Them thu vien Hang doi(trong c++)
...
//Ham tim xep trang thai co trong ngan xep khong?
	int FindState(state ST,queue<node> HD){
		while(!HD.empty()){//Su dung phuong thuc kiem tra ngan xep rong 
			if(compareSate(HD.front()->TH,ST) == 1){
				return 1;
			}else{
				HD.pop();
			}
...
//In Duong di
	void cachGiai(node P){
		...
		for(Q = P; Q != NULL; Q = Q->next){
			KQ[dodai].TH.x = Q->TH.x;
			KQ[dodai].TH.y = Q->TH.y;
			KQ[dodai].opt = Q->operation;
			//printf("\n[X]: %d -- [Y]: %d",KQ[dodai].TH.x,KQ[dodai].TH.y);
			dodai++;
		}
		int i;
		for(i=dodai-1;i>=0;i--){
			printf("\n%s: [X]: %d -- [Y]: %d",action[KQ[i].opt],KQ[i].TH.x,KQ[i].TH.y);
		}
	}
int main(){
	state h1;
